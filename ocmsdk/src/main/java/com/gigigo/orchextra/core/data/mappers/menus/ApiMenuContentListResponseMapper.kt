...
import com.gigigo.orchextra.core.data.mappers.toMenuContent
...
constructor() : ExternalClassToModelMapper<ApiMenuContentData, MenuContentData> {
  ...
  override fun externalClassToModel(data: ApiMenuContentData): MenuContentData {
    ...
    data.menuContentList?.let {
      for (apiMenuContent in data.menuContentList) {
        menuContentList.add(
            apiMenuContent.toMenuContent()) //apiMenuContentMapper.externalClassToModel(apiMenuContent)
      }
    }
